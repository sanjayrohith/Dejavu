...
async function requestJson(base, method, path, body) {
  ...
  for (let attempt = 1; attempt <= 4; attempt++) {
    let res; try { res = await fetch(base + path, { method, headers: body ? { "content-type": "application/json" } : {}, body: body ? JSON.stringify(body) : undefined }); } catch (error) { last = error; if (attempt < 4) { await new Promise(r=>setTimeout(r,150*attempt)); continue; } throw error; }
    const raw = await res.text(); let data;
    try { data = JSON.parse(raw); } catch { last = new Error(`${method} ${path}: ${res.status} non-json ${raw.slice(0, 300)}`); if (res.status >= 500 && attempt < 4) { await new Promise(r=>setTimeout(r, 150*attempt)); continue; } throw last; }
    if (!res.ok) { last = new Error(`${method} ${path}: ${res.status} ${JSON.stringify(data)}`); if (res.status >= 500 && attempt < 4) { await new Promise(r=>setTimeout(r, 150*attempt)); continue; } throw last; }
    return { data, ms: performance.now() - t0, attempts: attempt };
  }
  throw last;
}
export class LiveMirrorClient {
  ...
  async connect() {
    ...
    let res, last;
    for (let attempt=1; attempt<=4; attempt++) { try { res = await fetch(`${this.base}/stream?since=${this.revision()}`, { signal: ctrl.signal }); if (res.ok && res.body) break; last = new Error(`stream ${res.status}`); } catch (error) { last = error; } if (attempt<4) await new Promise(r=>setTimeout(r,200*attempt)); }
    if (!res?.ok || !res.body) throw last || new Error(`stream unavailable`);
    this.connected = true;
